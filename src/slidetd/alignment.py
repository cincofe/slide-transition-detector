...
from slidetd.mediaoutput import IncrementalImageWriter
from slidetd.slides import SlideDataHelper

class Aligner(object):
    
    def __init__(self, slides_descriptor, output, jump_probability, jump_range):
        self.originals = list(map(lambda x: x.img, SlideDataHelper(slides_descriptor).get_slides()))
        assert len(jump_range) == 2
        assert len(list(filter(lambda x: int(x) > 0, jump_range))) == 2
